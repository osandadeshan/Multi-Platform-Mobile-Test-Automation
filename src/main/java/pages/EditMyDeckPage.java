...
public class EditMyDeckPage extends BasePage {

    public static final String EDIT_MY_DECK_PAGE = "EditMyDeckPage";
    ...
    public EditMyDeckPage() {
        PageFactory.initElements(androidDriver, this);
        PageFactory.initElements(iosDriver, this);
    }

    public void navigateBackFromEditMyDeckPage() throws IOException {
        tapElement(EDIT_MY_DECK_PAGE, BACK_BUTTON);
    }

    public void remindMeSwitchStatus(String status) throws IOException {
        verifyElementAccessibilityIdTextIsVisibleAs(EDIT_MY_DECK_PAGE, REMIND_ME_SWITCH, status);
    }

    public void editMyDeck(String deckTitle, String examDate, String frequency, String hour, String minutes, String amPm) throws IOException {
        if (!deckTitle.isEmpty()){
            setTextAs(EDIT_MY_DECK_PAGE, DECK_TITLE_TEXTFIELD, deckTitle);
        }
        if (!examDate.isEmpty()){
            setDatePickerAndroid(EDIT_MY_DECK_PAGE, EXAM_DATE_DATE_PICKER, EXAM_DATE_ELEMENT, examDate, EXAM_DATE_DATE_PICKER_OK_BUTTON);
        }
        if (!frequency.isEmpty()){
            tapElement(EDIT_MY_DECK_PAGE, FREQUENCY_DROPDOWN);
            replaceXpathContentAndClickElement(EDIT_MY_DECK_PAGE, FREQUENCY_DROPDOWN_VALUES_LIST, "frequency", frequency);
        }
        ...
        if (!hour.isEmpty() && !minutes.isEmpty() && !amPm.isEmpty()){
            setTimePickerAndroid(EDIT_MY_DECK_PAGE, TIMER, AM_LABEL, PM_LABEL, hour, minutes, amPm, OK_BUTTON);
        }
        tapElement(EDIT_MY_DECK_PAGE, SAVE_BUTTON);
    }

    public void removeExamDate() throws IOException {
        tapElement(EDIT_MY_DECK_PAGE, REMOVE_EXAM_DATE);
    }

    public void removeDeckTitle() throws IOException {
        setTextAs(EDIT_MY_DECK_PAGE, DECK_TITLE_TEXTFIELD, "");
    }
    ...
    public void verifySetTime(String expectedTimeString) throws IOException {
        ...
        verifyElementAccessibilityIdTextIsVisibleAs(EDIT_MY_DECK_PAGE, TIMER, expectedTimeString);
    }

    public void resetTextFields() throws IOException {
        clearText(EDIT_MY_DECK_PAGE, DECK_TITLE_TEXTFIELD);
    }

    public void verifyInputFieldValue(String inputField, String text) throws IOException {
        isElementTextEquals(EDIT_MY_DECK_PAGE, inputField, text);
    }
    ...
    public void verifyFrequencyDropdownIsEnable(Boolean isEnabled) throws IOException {
        verifyElementIsEnable(EDIT_MY_DECK_PAGE, FREQUENCY_DROPDOWN, isEnabled);
    }

    public void verifyTimerIsEnable(Boolean isEnabled) throws IOException {
        verifyElementIsEnable(EDIT_MY_DECK_PAGE, TIMER, isEnabled);
    }

    public void verifyFrequencyValue(String expectedFrequency) throws IOException {
        isElementNameEqualTo(EDIT_MY_DECK_PAGE, SELECTED_FREQUENCY_VALUE, expectedFrequency);
    }
